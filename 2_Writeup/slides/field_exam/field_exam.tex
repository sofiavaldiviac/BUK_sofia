...
\vfill
\begin{center}
{\Large\bfseries\textcolor{navy}{Market Power and Gender Gaps in Career Dynamics}}\\[6pt]
...
\begin{itemize}
    \item Gender gaps persist despite convergence in education {\scriptsize\textcolor{muted}{(Goldin, 2014; Blau \& Kahn, 2017)}}, yet most research focuses on wages. Firms also decide \textbf{who gets hired} and \textbf{who gets promoted}.

    \medskip
    \item Search frictions give firms market power {\scriptsize\textcolor{muted}{(Manning, 2003)}}, and women face fewer outside options {\scriptsize\textcolor{muted}{(Webber, 2016; Caldwell \& Danieli, 2022)}}. \bmark{This depresses wages --- but does it also distort hiring and promotion decisions?}

    \medskip
    \item \textbf{The mechanism:} When deciding whether to promote, the firm weighs the output gain against the wage cost --- and retention. A worker with more outside options must be promoted to be retained; one with fewer can be delayed at little cost.

    \medskip
    \item \textbf{This paper:} connects market power to gender gaps in hiring and promotion, using matched employer-employee-applicant data from Chile.
\end{itemize}
...
\begin{itemize}
    ...
    \medskip
    \item These gaps have become a central policy concern, prompting a series of \textbf{labor market reforms}:
    \begin{itemize}
        \smallskip
    ...
    \medskip
    \item Each shifts a different \textbf{parameter of the firm's problem} and can be exploited for identification
\end{itemize}
...
\vspace{-0.1cm}
\begin{center}
\includegraphics[width=0.75\textwidth]{rf_6_4_reform_timeline.png}
\end{center}
...
\begin{itemize}\itemsep1pt
    ...
    \item Three reforms fall \textbf{within the panel window}. Art.\ 203 mandate throughout %, creating pre/post variation
...
\vspace{0.3cm}
\begin{itemize}
    \item \textbf{Hierarchy:} supervisor identifier enables org structure %and promotion measurement
    \item \textbf{Job characteristics:} area, standardized occupation, contract type (fixed-term / indefinite), workday type (full-time, part-time, Art.\ 22 exempt)
    \item \textbf{Separations:} end date for completed spells; exit reason (voluntary / involuntary)
\end{itemize}
...
\vspace{0.3cm}
\begin{itemize}
    \item Vacancy records functional area, salary range offered, and reason % (new position, replacement, temporary)
    \medskip
    \item Application has candidate demographics, referral source, and application score
    \medskip
...
\vspace{0.2cm}
\begin{itemize}
    \item Large firms over-represented $\Rightarrow$ \textbf{advantage}: model predictions on hiring and promotion are most testable in these firms %with structured pipelines
    \medskip
...
\end{itemize}
\hfill\hyperlink{coverage-heatmap}{\beamergotobutton{Coverage heatmap}}
...
\begin{enumerate}\itemsep4pt
    ...
    {\color{faded}\setbeamercolor{enumerate item}{fg=faded}\setbeamercolor{itemize subitem}{fg=faded}\setbeamercolor{normal text}{fg=faded}\usebeamercolor[fg]{normal text}%
    \item Bonuses vs.\ promotions as retention tools
    \begin{itemize}
        \item Do firms substitute variable pay (performance bonuses, commissions) for career advancement, especially for women?
    \end{itemize}

    \item The 40-hour workweek and flexible schedule workers
    \begin{itemize}
        \item Does restricting Art.\ 22 exemptions hurt workers who valued schedule flexibility --- particularly mothers?
    \end{itemize}

    \item The broken rung at first promotion
    \begin{itemize}
        \item Is there a gender gap at the first promotion, and do long-hours norms explain who gets promoted?
    \end{itemize}

    \item Wage growth over the life cycle
    \begin{itemize}
        \item How does market power shape wage trajectories by gender at different career stages?
    \end{itemize}}
\end{enumerate}
...
\begin{itemize}\itemsep3pt
    \item \textbf{Imperfect competition in labor markets:} Search frictions give firms market power; outside options shape wages and mobility {\scriptsize\textcolor{muted}{(Manning, 2003; Burdett \& Mortensen, 1998; Postel-Vinay \& Robin, 2002)}} --- \bmark{but the focus is on wages}
...
% ============================================================
% SECTION F: Preliminary Evidence
% ============================================================

% --- HHI distribution ---
\begin{frame}[label=hhislide]
...
\end{itemize}
\hfill\hyperlink{hhi-vacancy-uw}{\beamergotobutton{Vacancy (unwtd)}}
\hyperlink{hhi-empl-w}{\beamergotobutton{Empl.\ (wtd)}}
\hyperlink{hhi-vacancy-w}{\beamergotobutton{Vacancy (wtd)}}
...
% --- Appendix: HHI alternative measures ---
\begin{frame}[label=hhi-vacancy-uw]
\frametitle{HHI distribution: vacancy-based (unweighted)}

\begin{center}
\includegraphics[width=0.62\textwidth]{rf_2_4_hhi_distribution_vacancy_unweighted.png}
\end{center}

\hfill\hyperlink{hhislide}{\beamerreturnbutton{Back}}

\end{frame}

\begin{frame}[label=hhi-empl-w]
\frametitle{HHI distribution: employment-based (weighted)}

\begin{center}
\includegraphics[width=0.62\textwidth]{rf_2_4_hhi_distribution_employment.png}
\end{center}
...
\begin{frame}[label=hhi-vacancy-w]
\frametitle{HHI distribution: vacancy-based (weighted)}

\begin{center}
\includegraphics[width=0.62\textwidth]{rf_2_4_hhi_distribution_vacancy.png}
\end{center}
...
% --- Appendix: Coverage heatmap ---
\begin{frame}[label=coverage-heatmap]
\frametitle{Coverage varies by sector and firm size}

\vspace{-0.1cm}
\begin{columns}[T]
\begin{column}{0.52\textwidth}
\includegraphics[width=0.84\textwidth]{../../../../4_Code/images/heatmap_coverage.png}
\end{column}
\begin{column}{0.45\textwidth}
\small
\vspace{0.3cm}
...
\end{itemize}
\hfill\hyperlink{sampleslide}{\beamerreturnbutton{Back}}
\end{column}
\end{columns}
